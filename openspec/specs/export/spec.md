...
#### Scenario: PDF 导出入口归属档案详情
- **WHEN** 用户访问真实 `/record/:id` 档案详情页
- **THEN** 页面 SHALL 提供“导出 PDF”动作
- **AND** `/app` 工作区 SHALL NOT 提供“导出 PDF”动作

#### Scenario: PDF 导出文件命名
- **WHEN** 用户在 `/record/:id` 触发 PDF 导出
...
系统 SHALL 支持在正式病历档案页将时间线表格导出为 PNG 图片，使用 html2canvas 截图，文件名包含当前日期。

#### Scenario: PNG 导出入口归属档案详情
- **WHEN** 用户访问真实 `/record/:id` 档案详情页
- **THEN** 页面 SHALL 提供“导出 PNG”动作
- **AND** `/app` 工作区 SHALL NOT 提供“导出 PNG”动作

#### Scenario: PNG 导出文件命名
- **WHEN** 用户在 `/record/:id` 触发 PNG 导出
...
系统 SHALL 在正式病历档案页的导出过程中向用户展示 loading 状态，禁止重复触发导出操作。

#### Scenario: 导出按钮 loading
- **WHEN** 用户在 `/record/:id` 点击导出按钮且导出尚未完成
...
### Requirement: 导出失败处理
系统 SHALL 在正式病历档案页导出失败时向用户展示错误提示，不得静默失败。
...
The system SHALL keep formal PDF/PNG export behavior stable when `/record/:id` gains alternate views, including dossier, minimal TimelineTable, and Gantt views.
...
#### Scenario: TimelineTable view does not hijack export
- **WHEN** a user switches to the minimal TimelineTable view
- **THEN** the page SHALL NOT silently redirect the existing PDF/PNG export actions to a different DOM target
- **AND** existing dossier export behavior SHALL remain available when the user returns to dossier view
