...
- **Decision**: 删除 `ci-cd.yml`，改为 `ci.yml` 与 `cd.yml`。
...
- **Decision**:
  ...
  - `cd.yml` 触发：`push tags: v*`、`workflow_dispatch`
- **Rationale**: PR 需要及时收到检查反馈；生产部署不应跟普通 `main` push 绑定。

### 3. 生产发布必须来自 main

- **Decision**: `cd.yml` 在 tag 和手动触发两条路径上都校验“待部署 commit 必须可从 `origin/main` 到达”。
...
- `CLOUDFLARE_API_TOKEN` 仍必须由用户提供并写入 GitHub repo secret，否则新的 `cd.yml` 无法真正对 Cloudflare 发布。
...
- Cloudflare Pages 项目 API 返回：
  ...
  - `preview_deployment_setting = none`
- GitHub repo 配置中最终存在 `CLOUDFLARE_API_TOKEN` secret 后，`cd.yml` 可通过 tag 或手动触发成功发版。
