...
系统 SHALL 在 MVP 中仅围绕登录页面、临床工作区、档案详情三类 Web 页面组织界面结构，并要求这三类页面在 dark / light 主题间共享相同的壳层空间角色。
...
系统 SHALL 以 `docs/design/dark/` 与 `docs/design/light/` 中对应页面导出的 HTML / screenshot 作为页面结构实现的设计证据源，并将这些结构约束折叠为统一的壳层几何合同，而不是仅复制单页视觉结果。
