...
**Reason**: 模型切换不再只属于 Gemini 模型族；系统需要在同一个 LLM adapter 边界下支持 Gemini 与 DeepSeek。

**Migration**: 使用新增的 `支持服务端模型提供商切换` requirement。Gemini 仍作为允许 provider 保留，原有 `options.model` 语义迁移为当前 provider 下的模型名。
