...
- **AND** 登录页所有可见文案 SHALL 根据当前语言状态以单语方式渲染
...
#### Scenario: 登录页提供独立语言切换控制
- **WHEN** 系统渲染 `/login` 的 Dark 或 Light 页面
- **THEN** 页面 SHALL 在主题切换控件旁边提供独立语言切换入口
- **AND** 语言切换 MUST NOT 改变当前主题
...
#### Scenario: 主题与语言切换职责独立
- **WHEN** 用户切换主题或语言
- **THEN** 主题切换 SHALL 只影响视觉材质与设计系统映射
- **AND** 语言切换 SHALL 只影响可见文案语言
- **AND** 两者 MUST NOT 互相隐式改变对方状态
...
#### Scenario: 壳层组件不输出混合语言文案
- **WHEN** 壳层组件渲染导航标签、按钮标题、辅助说明或顶部 utility 文案
- **THEN** 壳层组件 SHALL 根据当前语言状态输出单语文案
- **AND** 它 MUST NOT 在同一字符串内混合中英文
