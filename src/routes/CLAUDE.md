...
record-page.tsx: 档案详情复刻实现，对应 /record/:id，单文件承载宽幅响应式档案版心、顶部概要、点击时动态加载的正式 PDF/PNG 导出入口、demo fallback 禁用态、8 项指标、纵向 01/02/03 时间轴、右侧证据卡与底部审计带，复用可变侧栏、匿名/非匿名会话身份展示与认证出口，[PROTOCOL]: 变更时更新此头部，然后检查 CLAUDE.md
record-page.test.tsx: 病例详情页响应式版心、背景音 provider 壳层依赖与导出职责回归测试，约束 /record/:id 使用共享宽幅 shell 合同、真实记录 PDF/PNG 可导出、demo fallback 禁用态并禁止回退到 980px 固定画布，[PROTOCOL]: 变更时更新此头部，然后检查 CLAUDE.md
