...
workspace-page.test.tsx: 工作区报告区、V3 单主按钮、移除侧栏状态卡、active 导航、病历/语言切换图标、隐藏恢复胶囊、左缘渐进拉出、拖拽到隐藏、紧凑默认弹出宽度与 locale 回归测试，约束 dark/light 都不再渲染多余总标题壳、废弃控制块、装饰性运行状态卡、active 卡片高亮、边缘亮条、folder 病历图标、泛化语言图标、下拉误导箭头、过宽隐藏恢复按钮、旧侧栏宽度缓存污染或双语漂移，[PROTOCOL]: 变更时更新此头部，然后检查 CLAUDE.md
record-page.tsx: 档案详情复刻实现，对应 /record/:id，单文件承载 V3 宽幅响应式档案版心、顶部概要、禁用态 PDF/PNG 导出入口、8 项指标、纵向 01/02/03 时间轴、右侧证据卡与底部审计带，复用可变侧栏与认证出口，[PROTOCOL]: 变更时更新此头部，然后检查 CLAUDE.md
record-page.test.tsx: 病例详情页响应式版心与导出职责回归测试，约束 /record/:id 使用共享宽幅 shell 合同、PDF/PNG demo fallback 禁用态并禁止回退到 980px 固定画布，[PROTOCOL]: 变更时更新此头部，然后检查 CLAUDE.md
