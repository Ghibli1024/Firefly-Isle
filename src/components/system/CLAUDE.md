...
topbar.tsx: dark/light 共享顶部状态条组件，定义页面名、系统就绪、帮助与设置入口的同构空间角色，[PROTOCOL]: 变更时更新此头部，然后检查 CLAUDE.md
masthead.tsx: light 旧版头兼容组件，内部转发到共享顶部状态条，不再定义独立 light 骨架，[PROTOCOL]: 变更时更新此头部，然后检查 CLAUDE.md
sidebar-nav.tsx: V3 响应式侧栏组件，统一 dark/light 的紧凑桌面默认展开、移动端默认收起、独立品牌 mark/标题、边线胶囊三态点击、窄恢复胶囊、左缘渐进拉出、拖拽缩放到隐藏、阈值 icon-only、active 图标与文字同色、Google Translate 语言图标、临床笔记病历图标、无下拉误导的主题/语言切换、隐私/匿名与会话出口，[PROTOCOL]: 变更时更新此头部，然后检查 CLAUDE.md
