...
demo-lab-analytics.ts: /analytics/demo 的演示实验室趋势数据源，压缩血常规/血生化/肿瘤指标三份表格为网页端 labResults fixture，复用 record demo 病例但不污染 /record/demo fixture，[PROTOCOL]: 变更时更新此头部，然后检查 CLAUDE.md
lab-analytics-dashboard.tsx: 实验室趋势统计主界面，编排中文摘要指标条、页面级图表编辑开关、可搜索/可滚动分类指标索引、趋势图控制、等价数据表、最近异常、肿瘤标志物连续上涨提醒联动高亮与 demo 标识，文件上传入口留在 /app 输入区，[PROTOCOL]: 变更时更新此头部，然后检查 CLAUDE.md
