...
login-entry-view.tsx: 登录页入口编排层，渲染双主题全屏背景、内页同源 FireflyBrandWordmark、route/stagger 进入节奏、小屏可纵向生长且随文档流/大屏右下固定的工具区、安全状态、登录 CTA、Demo CTA 与认证弹层入口，[PROTOCOL]: 变更时更新此头部，然后检查 CLAUDE.md
login-trace-map.tsx: 登录页双主题海岸背景模块，保留干净铺底、线性氛围遮罩与极慢背景呼吸，不承载业务操作，[PROTOCOL]: 变更时更新此头部，然后检查 CLAUDE.md
