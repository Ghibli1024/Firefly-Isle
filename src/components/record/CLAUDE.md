...
record-copy.ts: 病例详情静态文案模块，提供 labels、含癌种与体格占位的 summaryMetrics 与 BL/Ln 标记/补充资料/rail 时间段/每线 PFS 归一的逐线演示时间线组装，[PROTOCOL]: 变更时更新此头部，然后检查 CLAUDE.md
...
record-derived.ts: PatientRecord 展示派生层，把真实病历转换为含癌种、身高、体重、BMI 的概要指标与 BL/Ln 标记/补充资料/rail 时间段/每线 PFS 归一的紧凑时间线条目，避免基础信息重复进入 timeline，[PROTOCOL]: 变更时更新此头部，然后检查 CLAUDE.md
record-dossier.tsx: 病例详情主展示层，承载档案版心、概要指标、按需实验室趋势、桌面独立日期/PFS rail、移动卡内 PFS、证据卡 stagger、导出按钮与不可用态，页头、标题旁线别小字与补充资料行不重复已表达信息，[PROTOCOL]: 变更时更新此头部，然后检查 CLAUDE.md
