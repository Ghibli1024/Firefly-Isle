...
provider-settings.ts: LLM provider 设置客户端，负责调用 llm-proxy/settings 读取、保存、重置设置并过滤任何明文 key 回读，[PROTOCOL]: 变更时更新此头部，然后检查 CLAUDE.md
provider-settings.test.ts: provider 设置客户端协议测试，约束 preset/custom 保存、DELETE 重置与明文 key redaction，[PROTOCOL]: 变更时更新此头部，然后检查 CLAUDE.md
